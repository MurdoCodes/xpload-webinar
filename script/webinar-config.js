(function (window, adapter) {
  ...
  var config = sessionStorage.getItem('xprowebinarBed');
  var json;
  var serverSettings = {
    "protocol": protocol,
    "httpport": port,
  ...
  function assignStorage () {
    json = {
      ...
      "host": 'streammanager.xprowebinar.com',
      "port": 8554, // rtsp
      "stream1": "stream1",
      "stream2": "stream2",
      "app": "live",
    ...
    */
    sessionStorage.setItem('xprowebinarBed', JSON.stringify(json));
  }
  ...
  if (config) {
    try {
      ...
      if (json.version && json.version !== build_version) {
        ...
        sessionStorage.removeItem('xprowebinarBed');
        assignStorage();
  ...
  sessionStorage.setItem('xprowebinarServerSettings', JSON.stringify(serverSettings));
  return json;
